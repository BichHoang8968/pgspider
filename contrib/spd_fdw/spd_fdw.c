...
typedef struct Childinfo{
	...
	Agg		   *pAgg;	/* "Aggref" for Disable of aggregation
									 * push down servers */
} Childinfo;
...
typedef struct SpdFdwPrivate
{
	...
	int			agg_tuples;      /* Number of aggregation tuples from temp table */
	...
	PlannerInfo *spd_root;       /* Copyt of root planner info. This is used by aggregation pushdown.*/
...
spd_add_to_flat_tlist(List *tlist, List *exprs, List **mapping_tlist, List **mapping_orig_tlist, List **compress_tlist, int *child_uninum, Index sgref)
{
	...
	foreach(lc, exprs)
	{
			...
			(aggref->aggfnoid >= STD_MIN_OID && aggref->aggfnoid <= STD_MAX_OID))
		{
			...
			if (tempCount->aggtype <= INT8OID || tempSum->aggtype == NUMERICOID)
			{
...
spd_ParseUrl(char *url_str, SpdFdwPrivate * fdw_private)
{
	...
	if(tp != NULL)
		...
		return; /* Pattern4 */
	/*
     *  url_option = /sample test/code
	 *                ^ *tp position
     *  url_str =    /sample/test/code/
     *                      |---------| <-Throwing URL
	 *                      * <- This pointer is url_str[strlen(tp)+ 1]
     */
	if (tp != NULL)
	{
...
spd_create_child_url(int childnums, RangeTblEntry *r_entry, SpdFdwPrivate * fdw_private, char **new_underurl)
{
	...
	for (int i = 0; i < childnums; i++)
	{
		...
		{
			if (strcmp(temp_fdw->fdwname, SPDFRONT_FDW_NAME) != 0)
			{
...
spd_GetForeignRelSize(PlannerInfo *root, RelOptInfo *baserel, Oid foreigntableid)
{
	...
	childinfo = (Childinfo*)palloc(sizeof(Childinfo)*nums);

	for (int i = 0; i < nums; i++)
		childinfo[i].oid = oid[i];
	...
	{
		for (int i = 0; i < nums; i++)
			childinfo[i].child_table_alive = TRUE;
	}
...
static void
spd_makedivtlist(Expr *temp_expr,Aggref *aggref,List *newList,SpdFdwPrivate *fdw_private){
						 ...
						 RelOptInfo *input_rel, RelOptInfo *output_rel)
{
	...
	foreach(lc, spd_root->upper_targets[UPPERREL_GROUP_AGG]->exprs)
	{
		...
		{
			spd_makedivtlist(temp_expr,aggref,newList,fdw_private);
		}
		else
		{
	...
	if (in_fdw_private->childinfo != NULL)
	{
		...
		for(i=0; i<fdw_private->node_num; i++){
			...
			foreach(lc, root->upper_targets[UPPERREL_GROUP_AGG]->exprs)
			{
				...
				if (OIDCHECK(aggref->aggfnoid))
				{
					spd_makedivtlist((Expr*)aggref,aggref,newList,fdw_private);
				}
				else
				{
...
spd_GetForeignPaths(PlannerInfo *root, RelOptInfo *baserel, Oid foreigntableid)
{
	...
	for(i=0; i < fdw_private->node_num; i++){
		...
		childinfo->server_oid = server_oid;
	...
	for(i=0; i < fdw_private->node_num; i++){
		...
		PG_TRY();
		{
			...
			if (tmp->pathlist != NULL)
			{
				...
				temptlist = build_path_tlist((PlannerInfo *) childinfo[i].root, tmp_path);
			}
			else
...
spd_BeginForeignScan(ForeignScanState *node, int eflags)
{
		...
		if (fdw_private->pPseudoAggPushList || fdw_private->pPseudoAggList)
		{
			...
			for (i = 0; i < fdw_private->child_uninum; i++)
			{
				...
				if (!strcmpi(agg_command, "AVG") || !strcmpi(agg_command, "VARIANCE") || !strcmpi(agg_command, "STDDEV"))
				{
					...
					memcpy(attrs[i], node->ss.ss_ScanTupleSlot->tts_tupleDescriptor->attrs[org_attrincr],
						   sizeof(FormData_pg_attribute));
					if (attrs[i]->atttypid <= INT8OID || attrs[i]->atttypid == NUMERICOID)
					{
						attrs[i - 1]->atttypid = INT8OID;
						attrs[i]->atttypid = INT8OID;
	...
	Assert(fdw_private->dummy_base_rel_list->length == fdw_private->nThreads);
...
spd_exec_select(SpdFdwPrivate *fdw_private,StringInfo sql,TupleTableSlot *slot){
	int			ret;
	int			i,k;
	int			colid;
	int			mapping;
...
spd_spi_select_table(TupleTableSlot *slot, ForeignScanState *node, SpdFdwPrivate * fdw_private)
{
	...
	slot = spd_exec_select(fdw_private,0,slot);
	/* calc and set agg values */
...
spd_spi_select_tablecp(TupleTableSlot *slot, ForeignScanState *node, SpdFdwPrivate * fdw_private)
{
	...
	if (fdw_private->agg_num < fdw_private->agg_tuples)
			return spd_calc_aggvalues(fdw_private,fdw_private->agg_num,slot);
	else
		return NULL;
}
	...
	if (fdw_private->agg_query)
	{
		if (fdw_private->isFirst)
		{
			...
			for (;;)
			{
				...
				{
					elog(ERROR, "Child thread node tuple is NULL");
				}
	...
	for (i = 0; i < node_slot->tts_tupleDescriptor->natts; i++)
	{
		...
		if (strcmp(node_slot->tts_tupleDescriptor->attrs[i]->attname.data, SPDURL) == 0)
		{
			...
			else
			{
				 ...
				 * attribute
				 */
				psprintf(value, "/%s/", fs->servername);
			}
...
spd_BeginForeignModify(ModifyTableState *mtstate,
					   ...
					   int eflags)
{
	Oid			oid_server = modify_server_oid;
	FdwRoutine *fdwroutine;

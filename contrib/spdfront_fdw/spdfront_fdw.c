 ...
 * a ModifyTable node referencing a spdfront_fdw foreign table.  We store:
 *
...
/*
 * Execution state of a foreign scan using spdfront_fdw.
 */
 ...
 * SQL functions
 */
PG_FUNCTION_INFO_V1(spdfront_fdw_handler);
 ...
 */
Datum
spdfront_fdw_handler(PG_FUNCTION_ARGS)
{
	FdwRoutine *routine = makeNode(FdwRoutine);

	/* Functions for scanning foreign tables */
	routine->GetForeignRelSize = spdfrontGetForeignRelSize;
	routine->GetForeignPaths = spdfrontGetForeignPaths;
	routine->GetForeignPlan = spdfrontGetForeignPlan;
	routine->BeginForeignScan = spdfrontBeginForeignScan;
	routine->IterateForeignScan = spdfrontIterateForeignScan;
	routine->ReScanForeignScan = spdfrontReScanForeignScan;
	routine->EndForeignScan = spdfrontEndForeignScan;

	/* Functions for updating foreign tables */
	routine->AddForeignUpdateTargets = spdfrontAddForeignUpdateTargets;
	routine->PlanForeignModify = spdfrontPlanForeignModify;
	routine->BeginForeignModify = spdfrontBeginForeignModify;
	routine->ExecForeignInsert = spdfrontExecForeignInsert;
	routine->ExecForeignUpdate = spdfrontExecForeignUpdate;
	routine->ExecForeignDelete = spdfrontExecForeignDelete;
	routine->EndForeignModify = spdfrontEndForeignModify;
	routine->IsForeignRelUpdatable = spdfrontIsForeignRelUpdatable;
	routine->PlanDirectModify = spdfrontPlanDirectModify;
	routine->BeginDirectModify = spdfrontBeginDirectModify;
	routine->IterateDirectModify = spdfrontIterateDirectModify;
	routine->EndDirectModify = spdfrontEndDirectModify;

	/* Function for EvalPlanQual rechecks */
	routine->RecheckForeignScan = spdfrontRecheckForeignScan;
	/* Support functions for EXPLAIN */
	routine->ExplainForeignScan = spdfrontExplainForeignScan;
	routine->ExplainForeignModify = spdfrontExplainForeignModify;
	routine->ExplainDirectModify = spdfrontExplainDirectModify;

	/* Support functions for ANALYZE */
	routine->AnalyzeForeignTable = spdfrontAnalyzeForeignTable;

	/* Support functions for IMPORT FOREIGN SCHEMA */
	routine->ImportForeignSchema = spdfrontImportForeignSchema;

	/* Support functions for join push-down */
	routine->GetForeignJoinPaths = spdfrontGetForeignJoinPaths;

	/* Support functions for upper relation push-down */
	routine->GetForeignUpperPaths = spdfrontGetForeignUpperPaths;

	PG_RETURN_POINTER(routine);
}

/*
 * spdfrontGetForeignRelSize
					   ...
					   Plan *outer_plan)
{
	...
	if (IS_SIMPLE_REL(foreignrel))
	{
		 ...
		 * previously examined by spdfrontGetForeignPaths.  Possibly it'd be
	...
	else
	{
		 ...
		 * recheck is handled elsewhere --- see spdfrontGetForeignJoinPaths().
	...
	/* Remember remote_exprs for possible use by spdfrontPlanDirectModify */
...
spdfrontBeginForeignScan(ForeignScanState *node, int eflags)
{
	...
	fsstate->batch_cxt = AllocSetContextCreate(estate->es_query_cxt,
											   "spdfront_fdw tuple data",
											   ALLOCSET_DEFAULT_SIZES);
	fsstate->temp_cxt = AllocSetContextCreate(estate->es_query_cxt,
											  "spdfront_fdw temporary data",
...
/*
 * spdfrontAddForeignUpdateTargets
								...
								Relation target_relation)
{
	...
	/*
	 * In spdfront_fdw, what we need is the ctid, same as for a regular table.
	 */
...
/*
 * spdfrontIsForeignRelUpdatable
 *		Determine whether a foreign table supports INSERT, UPDATE and/or
 *		DELETE.
 */
static int
spdfrontIsForeignRelUpdatable(Relation rel)
{
	bool		updatable;
	ForeignTable *table;
	...
	/*
	 * By default, all spdfront_fdw foreign tables are assumed updatable. This
...
set_transmission_modes(void)
{
	...
	if (IntervalStyle != INTSTYLE_POSTGRES)
		(void) set_config_option("intervalstyle", "spdfront",
								 PGC_USERSET, PGC_S_SESSION,
								 GUC_ACTION_SAVE, true, 0, false);
					 ...
					 const char ***param_values)
{
	...
	*param_flinfo = (FmgrInfo *) palloc0(sizeof(FmgrInfo) * numParams);
	 ...
	 * benefit, and it'd require spdfront_fdw to know more than is desirable
							...
							BlockNumber *totalpages)
{
	...
	*func = spdfrontAcquireSampleRowsFunc;

	/*
	 * Now we have to get the number of pages.  It's annoying that the ANALYZE
	 * API requires us to return that now, because it forces some duplication
	 * of effort between this routine and spdfrontAcquireSampleRowsFunc.  But
...
/*
 * Acquire a random sample of rows from foreign table managed by spdfront_fdw.
 *
...
spdfrontImportForeignSchema(ImportForeignSchemaStmt *stmt, Oid serverOid)
{
	...
	PG_TRY();
	{
		...
		/* Partitions are supported since Spdfront 10 */

...
PG_FUNCTION_INFO_V1(tinybrace_fdw_validator);
...
tinybrace_fdw_validator(PG_FUNCTION_ARGS)
{
	...
	foreach(cell, options_list)
	{
		...
		if (!tinybrace_is_valid_option(def->defname, catalog))
		{
			struct TinyBraceFdwOption *opt;
			StringInfoData buf;
 ...
 */
bool
tinybrace_is_valid_option(const char *option, Oid context)
{
	struct TinyBraceFdwOption *opt;

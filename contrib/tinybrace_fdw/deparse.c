...
static char *tinybrace_quote_identifier(const char *s, char q);
						 ...
						 List * targetAttrs)
{
	...
	if (targetAttrs)
	{
		...
		foreach(lc, targetAttrs)
		{
			...
			tinybrace_deparse_column_ref(buf, rtindex, attnum, root);
		}
...
void
tinybrace_deparse_analyze(StringInfo sql, char *dbname, char *relname)
{
	...
	*retrieved_attrs = NIL;
	for (i = 1; i <= tupdesc->natts; i++)
	{
						  ...
						  attrs_used))
		{
			...
			tinybrace_deparse_column_ref(buf, rtindex, i, root);
...
deparseExpr(Expr * node, deparse_expr_cxt * context)
{
	...
	{
		case T_Var:
			tinybrace_deparse_var((Var *) node, context);
			break;
		case T_Const:
			tinybrace_deparse_const((Const *) node, context);
			break;
		case T_Param:
			tinybrace_deparse_param((Param *) node, context);
			break;
		case T_ArrayRef:
			tinybrace_deparse_array_ref((ArrayRef *) node, context);
			break;
		case T_FuncExpr:
			tinybrace_deparse_func_expr((FuncExpr *) node, context);
			break;
		case T_OpExpr:
			tinybrace_deparse_op_expr((OpExpr *) node, context);
			break;
		case T_DistinctExpr:
			tinybrace_deparse_distinct_expr((DistinctExpr *) node, context);
			break;
		case T_ScalarArrayOpExpr:
			tinybrace_deparse_scalar_array_op_expr((ScalarArrayOpExpr *) node, context);
			break;
		case T_RelabelType:
			tinybrace_deparse_relabel_type((RelabelType *) node, context);
			break;
		case T_BoolExpr:
			tinybrace_deparse_bool_expr((BoolExpr *) node, context);
			break;
		case T_NullTest:
			tinybrace_deparse_null_test((NullTest *) node, context);
			break;
		case T_ArrayExpr:
...
/*
 * Deparse Interval type into Tinybrace Interval representation.
 */
static void
...
tinybrace_deparse_param(Param * node, deparse_expr_cxt * context)
{
	if (context->params_list)
	{
		...
		tinybrace_print_remote_param(pindex, node->paramtype, node->paramtypmod, context);
	}
	else
	{
		tinybrace_print_remote_placeholder(node->paramtype, node->paramtypmod, context);
	}
}
...
tinybrace_deparse_func_expr(FuncExpr * node, deparse_expr_cxt * context)
{
	...
	/* Translate PostgreSQL function into tinybrace function */
	proname = tinybrace_replace_function(NameStr(procform->proname));
...
tinybrace_deparse_op_expr(OpExpr * node, deparse_expr_cxt * context)
{
	...
	tinybrace_deparse_operator_name(buf, form);
 ...
 */
static void
tinybrace_deparse_operator_name(StringInfo buf, Form_pg_operator opform)
{

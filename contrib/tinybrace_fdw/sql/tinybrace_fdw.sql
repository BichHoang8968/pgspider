...
CREATE SERVER testserver1 FOREIGN DATA WRAPPER tinybrace_fdw;
DO $d$
    BEGIN
        EXECUTE $$CREATE SERVER loopback FOREIGN DATA WRAPPER tinybrace_fdw
            OPTIONS (dbname '$$||current_database()||$$',
                     IP   '127.0.0.1',
                     port '5100'
            )$$;
        EXECUTE $$CREATE SERVER loopback2 FOREIGN DATA WRAPPER tinybrace_fdw
            OPTIONS (dbname '$$||current_database()||$$',
                     IP   '127.0.0.1',
                     port '5100'
            )$$;
...
ALTER SERVER loopback OPTIONS (ADD extensions 'tinybrace_fdw');
...
alter server loopback options (set extensions 'tinybrace_fdw');
...
explain (verbose, costs off)
select count(t1.c3) from ft1 t1, ft1 t2 where t1.c1 = tinybrace_fdw_abs(t1.c2);
...
CREATE SERVER fetch101 FOREIGN DATA WRAPPER tinybrace_fdw OPTIONS( fetch_size '101' );

...
CREATE FOREIGN TABLE department(department_id int OPTIONS (key 'true'), department_name text) SERVER tinybrace_svr;
CREATE FOREIGN TABLE employee(emp_id int OPTIONS (key 'true'), emp_name text, emp_dept_id int) SERVER tinybrace_svr;
CREATE FOREIGN TABLE empdata(emp_id int OPTIONS (key 'true'), emp_dat bytea) SERVER tinybrace_svr;
CREATE FOREIGN TABLE numbers(a int OPTIONS (key 'true'), b varchar(255)) SERVER tinybrace_svr;
CREATE FOREIGN TABLE multiprimary(a int, b int OPTIONS (key 'true'), c int OPTIONS(key 'true')) SERVER tinybrace_svr;
...
INSERT INTO empdata  VALUES(1, decode ('01234567', 'base64'));
...
UPDATE empdata SET emp_dat = decode ('0123', 'he');
SELECT * FROM empdata;
...
DROP USER MAPPING FOR public SERVER tinybrace_svr;

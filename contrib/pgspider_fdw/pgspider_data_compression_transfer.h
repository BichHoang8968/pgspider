 ...
 * It is used in data compression feature protocol and sent to Function side via REST API request body.
 * When supporting new data source, new target DB value should be added at last place of this enum
 * (both PGSpider and Function side).
 */
...
typedef void (*PrepareDDLRequestDataFunc) (StringInfo jsonBody,
										   int mode,
										   Relation rel,
										   DDLData ddlData,
										   DataCompressionTransferOption * dct_option);
typedef void (*PrepareInsertRequestData) (StringInfo jsonBody,
...
typedef struct DataSource
{
	const char				   *fdw_name;
	TargetDB					targetDB;
	PrepareDDLRequestDataFunc	DDLFunc;
	PrepareInsertRequestData	InsertFunc;

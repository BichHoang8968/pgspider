...
CREATE SERVER testserver1 FOREIGN DATA WRAPPER pgspider_fdw;
DO $d$
    BEGIN
        EXECUTE $$CREATE SERVER loopback FOREIGN DATA WRAPPER pgspider_fdw
            OPTIONS (dbname '$$||current_database()||$$',
                     port '$$||current_setting('port')||$$'
            )$$;
        EXECUTE $$CREATE SERVER loopback2 FOREIGN DATA WRAPPER pgspider_fdw
            OPTIONS (dbname '$$||current_database()||$$',
                     port '$$||current_setting('port')||$$'
            )$$;
        EXECUTE $$CREATE SERVER loopback3 FOREIGN DATA WRAPPER pgspider_fdw
            OPTIONS (dbname '$$||current_database()||$$',
                     port '$$||current_setting('port')||$$'
            )$$;
    END;
...
CREATE FUNCTION pgspider_fdw_abs(int) RETURNS int AS $$
BEGIN
RETURN abs($1);
END
...
EXPLAIN (VERBOSE, COSTS OFF)
  SELECT count(c3) FROM ft1 t1 WHERE t1.c1 = pgspider_fdw_abs(t1.c2);
SELECT count(c3) FROM ft1 t1 WHERE t1.c1 = pgspider_fdw_abs(t1.c2);
EXPLAIN (VERBOSE, COSTS OFF)
  SELECT count(c3) FROM ft1 t1 WHERE t1.c1 === t1.c2;
SELECT count(c3) FROM ft1 t1 WHERE t1.c1 === t1.c2;
...
ALTER EXTENSION pgspider_fdw ADD FUNCTION pgspider_fdw_abs(int);
ALTER EXTENSION pgspider_fdw ADD OPERATOR === (int, int);
ALTER SERVER loopback OPTIONS (ADD extensions 'pgspider_fdw');
...
EXPLAIN (VERBOSE, COSTS OFF)
SELECT t1.c1, t2.c2, t1.c3 FROM ft1 t1 FULL JOIN ft2 t2 ON (t1.c1 = t2.c1) WHERE pgspider_fdw_abs(t1.c1) > 0 OFFSET 10 LIMIT 10;
...
alter extension pgspider_fdw add function least_accum(anyelement, variadic anyarray);
alter extension pgspider_fdw add aggregate least_agg(variadic items anyarray);
alter server loopback options (set extensions 'pgspider_fdw');
...
alter extension pgspider_fdw drop function least_accum(anyelement, variadic anyarray);
alter extension pgspider_fdw drop aggregate least_agg(variadic items anyarray);
alter server loopback options (set extensions 'pgspider_fdw');
...
-- Add into extension
alter extension pgspider_fdw add operator class my_op_class using btree;
alter extension pgspider_fdw add function my_op_cmp(a int, b int);
alter extension pgspider_fdw add operator family my_op_family using btree;
alter extension pgspider_fdw add operator public.<^(int, int);
alter extension pgspider_fdw add operator public.=^(int, int);
alter extension pgspider_fdw add operator public.>^(int, int);
alter server loopback options (set extensions 'pgspider_fdw');
...
alter extension pgspider_fdw drop operator class my_op_class using btree;
alter extension pgspider_fdw drop function my_op_cmp(a int, b int);
alter extension pgspider_fdw drop operator family my_op_family using btree;
alter extension pgspider_fdw drop operator public.<^(int, int);
alter extension pgspider_fdw drop operator public.=^(int, int);
alter extension pgspider_fdw drop operator public.>^(int, int);
alter server loopback options (set extensions 'pgspider_fdw');
...
EXPLAIN (verbose, costs off)
UPDATE ft2 SET c3 = 'bar' WHERE pgspider_fdw_abs(c1) > 2000 RETURNING *;            -- can't be pushed down
UPDATE ft2 SET c3 = 'bar' WHERE pgspider_fdw_abs(c1) > 2000 RETURNING *;
EXPLAIN (verbose, costs off)
...
CREATE SERVER fetch101 FOREIGN DATA WRAPPER pgspider_fdw OPTIONS( fetch_size '101' );
...
GRANT USAGE ON FOREIGN DATA WRAPPER pgspider_fdw TO regress_nosuper;
...
DO $d$
    BEGIN
        EXECUTE $$CREATE SERVER loopback_nopw FOREIGN DATA WRAPPER pgspider_fdw
            OPTIONS (dbname '$$||current_database()||$$',
                     port '$$||current_setting('port')||$$'
            )$$;
    END;
...
-- =============================================================================
-- test connection invalidation cases and pgspider_fdw_get_connections function
-- =============================================================================
-- Let's ensure to close all the existing cached connections.
SELECT 1 FROM pgspider_fdw_disconnect_all();
-- No cached connections, so no records should be output.
SELECT server_name FROM pgspider_fdw_get_connections() ORDER BY 1;
...
SELECT * FROM pgspider_fdw_get_connections() ORDER BY 1;
...
-- =======================================================================
-- test pgspider_fdw_disconnect and pgspider_fdw_disconnect_all functions
-- =======================================================================
BEGIN;
...
-- can not be closed.
SELECT pgspider_fdw_disconnect('loopback');
-- List all the existing cached connections. loopback and loopback2 should be
-- output.
SELECT server_name FROM pgspider_fdw_get_connections() ORDER BY 1;
...
SELECT pgspider_fdw_disconnect_all();
RESET client_min_messages;
COMMIT;
-- Ensure that loopback2 connection is closed.
SELECT 1 FROM pgspider_fdw_disconnect('loopback2');
SELECT server_name FROM pgspider_fdw_get_connections() WHERE server_name = 'loopback2';
-- Return false as loopback2 connection is closed already.
SELECT pgspider_fdw_disconnect('loopback2');
-- Return an error as there is no foreign server with given name.
SELECT pgspider_fdw_disconnect('unknownserver');
-- Let's ensure to close all the existing cached connections.
SELECT 1 FROM pgspider_fdw_disconnect_all();
-- No cached connections, so no records should be output.
SELECT server_name FROM pgspider_fdw_get_connections() ORDER BY 1;
...
CREATE SERVER batch10 FOREIGN DATA WRAPPER pgspider_fdw OPTIONS( batch_size '10' );
...
CREATE SERVER inv_scst FOREIGN DATA WRAPPER pgspider_fdw
...
ALTER FOREIGN DATA WRAPPER pgspider_fdw OPTIONS (nonexistent 'fdw');

-- ===================================================================
-- test pgspider_fdw.application_name GUC
-- ===================================================================
...
-- pgspider_fdw.application_name overrides application_name option
-- of a server object if both settings are present.
SET pgspider_fdw.application_name TO 'fdw_%a%u%%';
SELECT 1 FROM ft6 LIMIT 1;
SELECT pg_terminate_backend(pid, 180000) FROM pg_stat_activity
  WHERE application_name =
...
SET pgspider_fdw.application_name TO 'fdw_%C%c';
SELECT 1 FROM ft6 LIMIT 1;
SELECT pg_terminate_backend(pid, 180000) FROM pg_stat_activity
  WHERE application_name =
...
--Clean up
RESET pgspider_fdw.application_name;
RESET debug_discard_caches;

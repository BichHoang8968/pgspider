...
typedef struct ConnCacheEntry
{
	...
	PGSpiderFdwConnState state; /* extra per-connection state */
} ConnCacheEntry;
 ...
 * SQL functions
 */
PG_FUNCTION_INFO_V1(pgspider_fdw_get_connections);
PG_FUNCTION_INFO_V1(pgspider_fdw_disconnect);
PG_FUNCTION_INFO_V1(pgspider_fdw_disconnect_all);
 ...
 */
PGconn *
PGSpiderGetConnection(UserMapping *user, bool will_prep_stmt, PGSpiderFdwConnState * *state)
{
	bool		found;
	bool		retry = false;
	...
	/*
	 * Set flag that we did PGSpiderGetConnection during the current
	 * transaction
	 */
	...
	PG_TRY();
	{
		...
		if (entry->state.pendingAreq)
			pgspider_process_pending_request(entry->state.pendingAreq);
	...
	PG_CATCH();
	{
		 ...
		 * in pgspiderfdw_report_error(). In this case, since PQstatus()
		 * indicates CONNECTION_BAD, checking only PQstatus() causes the false
		 * detection of connection failure. To avoid this, we also verify that
		 * the error's sqlstate is ERRCODE_CONNECTION_FAILURE. Note that also
		 * checking only the sqlstate can cause another false detection
		 * because pgspiderfdw_report_error() may report
		 * ERRCODE_CONNECTION_FAILURE for any libpq-originated error
		 * condition.
		 */
...
make_new_connection(ConnCacheEntry *entry, UserMapping *user)
{
	...
	elog(DEBUG3, "new pgspider_fdw connection %p for server \"%s\" (user mapping oid %u, userid %u)",
...
configure_remote_session(PGconn *conn)
{
	 ...
	 * logic should match what pg_dump does.  See also
	 * pgspider_set_transmission_modes in pgspider_fdw.c.)
	 */
 ...
 */
PGresult *
pgspiderfdw_exec_query(PGconn *conn, const char *query, PGSpiderFdwConnState * state)
{
	/* First, process a pending asynchronous request, if any. */
	if (state && state->pendingAreq)
		pgspider_process_pending_request(state->pendingAreq);
...
pgfdw_xact_callback(XactEvent event, void *arg)
{
	...
	while ((entry = (ConnCacheEntry *) hash_seq_search(&scan)))
	{
				...
				case XACT_EVENT_PRE_PREPARE:
					...
					ereport(ERROR,
							(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
							 errmsg("cannot PREPARE a transaction that has operated on pgspider_fdw foreign tables")));
					break;
		 ...
		 * discard it to recover. Next PGSpiderGetConnection will open a new
		 * connection.
		 */
 ...
 * pgspider_fdw from the local session to the foreign server with
 ...
 */
Datum
pgspider_fdw_disconnect(PG_FUNCTION_ARGS)
{
	ForeignServer *server;
	char	   *servername;
 ...
 * pgspider_fdw from the local session to the foreign servers.
 ...
 */
Datum
pgspider_fdw_disconnect_all(PG_FUNCTION_ARGS)
{

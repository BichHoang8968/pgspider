...
sqlite_convert_to_pg(Oid pgtyp, int pgtypmod, sqlite3_stmt *stmt, int attnum)
{
	...
	switch (pgtyp)
		{
		/*
		 * Sqlite gives BIT / BIT(n) data type as decimal value. The only way to
		 * retrieve this value is to use BIN, OCT or HEX function in Sqlite, otherwise
		 * sqlite client shows the actual decimal value, which could be a non - printable character.
		 * For exmple in Sqlite
		 *
		 ...
		 * +--------+
		 *
		 * PostgreSQL expacts all binary data to be composed of either '0' or '1'. Sqlite gives

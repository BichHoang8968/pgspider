...
CREATE FOREIGN TABLE hist(time timestamp with time zone, iid text,
q bigint, ts bigint, v float8, vr text, __spd_url text) SERVER pgspider_svr;

CREATE FOREIGN TABLE hist__server1__0(time timestamp with time zone, iid text,
q bigint, ts bigint, v float8, vr text, __spd_url text) SERVER server1 OPTIONS(table 'hist');


-- query rewritten for postgres
EXPLAIN (VERBOSE)
SELECT iid, influx_time(time, interval '1h', interval '1451606400.000001s') AS tm, 
LAST(time,v) AS v, LAST(time,q) AS q, LAST(time,vr) AS vr FROM hist WHERE
 ...
 GROUP BY influx_time(time, interval '1h', interval '1451606400.000001s'),iid ORDER BY iid,tm;
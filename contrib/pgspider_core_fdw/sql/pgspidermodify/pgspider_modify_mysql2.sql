...
-- key: c1, c2
--Testcase 8:
CREATE SERVER mysql_svr1 FOREIGN DATA WRAPPER mysql_fdw OPTIONS (host :MYSQL_SERVER, port :MYSQL_PORT);
CREATE SERVER mysql_svr2 FOREIGN DATA WRAPPER mysql_fdw OPTIONS (host :MYSQL_SERVER, port :MYSQL_PORT);
--Testcase 9:
CREATE USER mapping for public SERVER mysql_svr1 OPTIONS(username :MYSQL_USER, password :MYSQL_PASS);
CREATE USER mapping for public SERVER mysql_svr2 OPTIONS(username :MYSQL_USER, password :MYSQL_PASS);
...
-- Foreign tables
--Testcase 11:
CREATE FOREIGN TABLE tntbl1__mysql_svr1__0 (c1 int, c2 smallint, c3 float, c4 double precision, c5 bigint, c6 timestamp, c7 timestamp with time zone, c8 char(30), c9 varchar(50)) SERVER mysql_svr1 OPTIONS (dbname :MYSQL_DB_NAME1, table_name 'tntbl1');
CREATE FOREIGN TABLE tntbl1__mysql_svr2__0 (c1 int, c2 smallint, c3 float, c4 double precision, c5 bigint, c6 timestamp, c7 timestamp with time zone, c8 char(30), c9 varchar(50)) SERVER mysql_svr2 OPTIONS (dbname :MYSQL_DB_NAME2, table_name 'tntbl1');

-- Reset data of remote table before test
DELETE FROM tntbl1__mysql_svr1__0;
DELETE FROM tntbl1__mysql_svr2__0;
...
-- Mysql stores floating-point value as approximate, so comparison is not correct.
-- No row is updated.
--Testcase 32:
UPDATE tntbl1 AS t SET c2 = 10 WHERE CAST(t.c3 AS DECIMAL(5, 2)) = 902.12;
--Testcase 33:
SELECT c1, c2, c3, c4, c5, c6, c7, c8, c9 FROM tntbl1 ORDER BY 1, 2, 3;
--Testcase 34:
...
--
--Testcase 36:
UPDATE tntbl1 SET c2=v.i FROM (VALUES(20, 0)) AS v(i, j)
  WHERE tntbl1.c2 = v.j;
...
--Testcase 38:
UPDATE tntbl1 SET c2 = v.* FROM (VALUES(30, 0)) AS v(i, j)
  WHERE tntbl1.c2 = v.j;
...
--Testcase 41:
UPDATE tntbl1 SET (c8,c5,c2) = ('bugle', c1+11, DEFAULT) WHERE c9 = 'foo';
--Testcase 42:
SELECT c1, c2, c3, c4, c5, c6, c7, c8, c9 FROM tntbl1 ORDER BY 1, 2, 3;
--Testcase 43:
...
--Testcase 46:
UPDATE tntbl1
  SET (c2,c3) = (SELECT c2,c3 FROM tntbl1 where c3 = 3 and c9 = 'car')
  WHERE c1 = 100 AND c2 = 20.0;
--Testcase 47:
SELECT c1, c2, c3, c4, c5, c6, c7, c8, c9 FROM tntbl1 ORDER BY 1, 2, 3;

-- correlated sub-SELECT:
-- Bug of mysql_fdw, temporally comment out
--Testcase 48:
-- UPDATE tntbl1 o
...
--Testcase 50:
UPDATE tntbl1 SET (c5,c2) = (SELECT c2+1,c1 FROM tntbl1);
...
--Testcase 53:
UPDATE tntbl1 SET (c5,c2) = ROW(v.*) FROM (VALUES(2, 100)) AS v(i, j)
  WHERE tntbl1.c2 = v.i;
-- you might expect this to work, but syntactically it's not a RowExpr:
--Testcase 54:
UPDATE tntbl1 SET (c5,c2) = (v.*) FROM (VALUES(2, 101)) AS v(i, j)
  WHERE tntbl1.c2 = v.i;
...
-- Bug crash, temporally comment out
--Testcase 59:
-- UPDATE tntbl1 t
--   SET (c2, c3) = (SELECT c3, c2 FROM tntbl1 s WHERE s.c2 = t.c2)
--   WHERE CURRENT_USER = SESSION_USER;
...
--
--Testcase 61:
UPDATE tntbl1 IN ('/mysql_svr/', '/sqlite_svr/') SET c3 = 22.2;
--
-- DELETE
--
...
--
--Testcase 67:
DELETE FROM tntbl1 IN ('/mysql_svr/') WHERE c1 = 20;
...
--Testcase 72:
INSERT INTO tntbl1 VALUES (120, 70, 2.0, 200.0, 3000, '2030-02-20 03:00:07', '2005-10-20 10:20:54+02', DEFAULT, 'abc');
--Testcase 73:
SELECT * FROM tntbl1 ORDER BY 1, 2, 3;
--Testcase 74:
INSERT INTO tntbl1 VALUES (100, 20, 3.0, 300.0, 4000, '2037-01-19 03:14:07', '2004-10-19 10:23:54+02', interval '2 months ago', '40', 'abc'), (-100, 30, DEFAULT, DEFAULT, 2000, '2004-10-19 10:23:54', '1972-01-01 00:00:00+07', interval '1 year 3 hours 20 minutes', 'Beera', 'John Does'),
    ((SELECT 200), (SELECT i FROM (VALUES(3)) as foo (i)), 250, 400.212, 5000, '2007-10-19 10:23:54', '2007-10-19 10:23:54+02', interval '1 year 2 months 3 days', 'VALUES are fun!', 'foo');
...
--Testcase 99:
EXPLAIN (VERBOSE, COSTS OFF)
UPDATE rw_view SET c2 = c2 + 100;
--Testcase 100:
UPDATE rw_view SET c2 = c2 + 100; -- should fail
--Testcase 101:
EXPLAIN (VERBOSE, COSTS OFF)
...
ALTER SERVER mysql_svr1 OPTIONS (ADD batch_size '10');
ALTER SERVER mysql_svr2 OPTIONS (ADD batch_size '10');
...
--Testcase 119:
SELECT * FROM tntbl1__mysql_svr1__0 ORDER BY 1,2;
--Testcase 120:
SELECT * FROM tntbl1__mysql_svr2__0 ORDER BY 1,2;

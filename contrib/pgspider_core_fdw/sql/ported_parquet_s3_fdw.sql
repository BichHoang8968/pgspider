...
SELECT * FROM test1;
SELECT * FROM test1 IN ('/parquet_s3_svr/') ORDER BY seven,__spd_url;
...
SELECT * FROM test1 WHERE one < 1;
SELECT * FROM test1 WHERE one <= 1;
SELECT * FROM test1 WHERE one > 6;
SELECT * FROM test1 WHERE one >= 6;
SELECT * FROM test1 WHERE one = 2;
SELECT * FROM test1 WHERE one = 7;
SELECT * FROM test1 WHERE six = true;
SELECT * FROM test1 WHERE six = false;
SELECT * FROM test1 WHERE seven < 0.9;
SELECT * FROM test1 WHERE seven IS NULL;
...
CREATE FOREIGN TABLE example_seq (
    ...
    seven   FLOAT8)
SERVER parquet_s3_svr
OPTIONS (filename 's3://ported/ported_3.parquet s3://ported/ported_2.parquet s3://ported/ported_1.parquet');
...
CREATE FOREIGN TABLE test2__parquet_s3_svr__1 (
    one     INT8,
    two     INT8[],
    three   TEXT,
    four    TIMESTAMP,
...
CREATE FOREIGN TABLE test2__parquet_s3_svr__2 (
    one     INT8,
    two     INT8[],
    three   TEXT,
    four    TIMESTAMP,
...
EXPLAIN (COSTS OFF) SELECT * FROM test2 ORDER BY one, five;
SELECT * FROM test2 ORDER BY one, five;

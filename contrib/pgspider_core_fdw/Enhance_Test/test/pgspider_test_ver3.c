    ...
    while(fgets(chline, 1024, pFile) != NULL) {
        if (strstr(chline, "Testcase"))
        {
            if (fgets(query, 2048, pFile) != NULL){
                ...
                // output Testcase to file
                printf(chline);
                chline[strlen(chline) - 1] = '\0';
                sprintf(command, "echo %s >> %s/../results/%s.txt", chline, dirname, filename);
                system(command);
                // output test case to tmp_query, the content like this:
                /*
                \o /dev/null
                set timezone to 0;
                \o
                ...
                SQL QUERY
                */
                fprintf(tmp_query, "\\o /dev/null\n%s\n\\o\n\n\\set ECHO all\n%s", settimezone, query);
                fclose(tmp_query);
                if (imodel == 1){
                    if (!connect_to_server(port1))
                        continue;
                    sprintf(command, "%s/PGS%s/bin/psql --host=127.0.0.1 --port=%s  --dbname=pgspider -f %s >> %s/../results/%s.txt 2>&1", DESTINATION_FOLDER, port1, port1, tmpfile, dirname, filename);
                }
                else if (imodel == 2){
                    if (!connect_to_server(port1))
                        continue;
                    ...
                    if (strstr(filename, "PGSpider1")){
                        ...
                        if (!connect_to_server(port2))
                            continue;
                        sprintf(command, "%s/PGS%s/bin/psql --host=127.0.0.1 --port=%s  --dbname=pgspider -f %s >> %s/../results/%s.txt 2>&1", DESTINATION_FOLDER, port2, port2, tmpfile, dirname, filename);
                    }
                    else if (strstr(filename, "PGSpider3")){
                        if (!connect_to_server(port3))
                            continue;
                        sprintf(command, "%s/PGS%s/bin/psql --host=127.0.0.1 --port=%s  --dbname=pgspider -f %s >> %s/../results/%s.txt 2>&1", DESTINATION_FOLDER, port3, port3, tmpfile, dirname, filename);
                    }
                }

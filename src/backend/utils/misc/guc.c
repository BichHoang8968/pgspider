...
#ifdef PGSPIDER
static void copy_guc_variables(void);
#endif
...
#ifdef PGSPIDER
static __thread struct config_bool ConfigureNamesBool[] =
#else
...
#ifdef PGSPIDER
static __thread struct config_int ConfigureNamesInt[] =
#else
...
#ifdef PGSPIDER
static __thread struct config_real ConfigureNamesReal[] =
#else
...
#ifdef PGSPIDER
static __thread struct config_string ConfigureNamesString[] =
#else
...
#ifdef PGSPIDER
static __thread struct config_enum ConfigureNamesEnum[] =
#else
...
static struct config_generic **main_guc_variables_copy;
#else
...
static int num_main_guc_variables;
#else
...
AtEOXact_GUC(bool isCommit, int nestLevel)
{
	...
	/*
	 * Each thread need to wait while other thread process and pop each stack
	 * entry within the nest level.
	 */
...
#ifdef PGSPIDER
/* This function is used to set a copy of guc_variables array of main thread */
void
copy_main_guc_variables(void)
{
	main_guc_variables_copy = guc_variables;
	num_main_guc_variables = num_guc_variables;
}

/*
 * Copy data of guc_variables of main thread. We can not call InitializeOneGUCOption
 * to create a completely new guc_variable for child thread because it calls some hook
 * functions which refers to some global variables. Child thread only need values so we
 * copy directly from main thread.
 */
static void
copy_guc_variables(void)
{
	/*
	 * Need mutex to avoid concurrency problem when main thread trying to access guc_variables
	 * while child thread is copying value.
	 */
	SPD_LOCK_TRY(&guc_mutex);
	...
	{
		struct config_generic *gconf = guc_variables[i];
		const char *name = gconf->name;
		const char **nameAddr = &name;
		struct config_generic **main_gconf;

		/*
		* Find corresponding option in guc_variables of main thread
		*/
		main_gconf = (struct config_generic **) bsearch((void *) &nameAddr,
												(void *) main_guc_variables_copy,
												num_main_guc_variables,
												sizeof(struct config_generic *),
												guc_var_compare);

		/* Copy current value of main thread */
		switch (gconf->vartype)
		{
			case PGC_BOOL:
			{
...
void
build_guc_variables_child_thread(void)
{
	build_guc_variables();
	copy_guc_variables();
}

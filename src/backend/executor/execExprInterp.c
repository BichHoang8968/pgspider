...
ExecEvalParamExec(ExprState *state, ExprEvalStep *op, ExprContext *econtext)
{
	 ...
	 *
	 * In PGSPider has mutiple threads which access the parameter at the same
	 * time, so other thread need to wait to get parameter before first thread
	 * evaluated the parameter.
	 */
